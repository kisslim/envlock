...
- user MUST NOT use ANY `conda` command, especially use that command to install ANY python packages. only `pip` is allowed to use. user MAY install provided wheels.
...
- user MAY use a proxy to `conda` command to install binaries they need, including java, javac, rustc, nodejs, npm, ollama, ..... the tool MUST choose the MOST POPULAR package manager for each language/toolchain. the user CAN NOT change the tool's choice.
